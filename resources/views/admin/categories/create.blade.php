                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.categories.store') }}">
                                    ...
                                    @foreach($languages as $index => $language)
                                        <div class="tab-content {{ $index === 0 ? '' : 'hidden' }} mt-4" id="lang-{{ $language->code }}">
                                            ...
                                            <input type="text" name="name[{{ $language->code }}]" id="name_{{ $language->code }}" 
                                                value="{{ old("name.{$language->code}") }}" required
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('admin.categories.index') }}" class="mr-4 text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100">Cancel</a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (defaultLangNameInput && slugInput) {
                ...
                // Generate slug on page load if slug is empty but name is filled
                if (!slugInput.value && defaultLangNameInput.value) {
                    generateSlug(defaultLangNameInput.value, slugInput);
                }
            }
