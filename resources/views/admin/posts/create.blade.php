<x-app-layout>
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.posts.store') }}" enctype="multipart/form-data" id="postForm" novalidate>
                                        ...
                                        <div class="tab-content {{ $index === 0 ? '' : 'hidden' }} mt-4" id="lang-{{ $language->code }}">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <textarea name="content[{{ $language->code }}]" id="content_{{ $language->code }}" rows="10" data-required="true"
                                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white editor">{{ old("content.{$language->code}") }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            @if(isset($tags) && $tags->count() > 0)
                                <select name="tags[]" id="tags" multiple
                                    class="tags-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                    <option value="">Select tags</option>
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', [])) ? 'selected' : '' }}>{{ $tag->getName() }}</option>
                                    @endforeach
                                </select>
                            @else
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No tags yet. <a href="{{ route('admin.tags.create') }}" class="text-indigo-600 dark:text-indigo-400">Create tags</a> first.</p>
                            @endif
                            @error('tags.*')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('admin.posts.index') }}" class="mr-4 text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100">Cancel</a>
    ...
    <style>
        ...
        .choices__inner { border-radius: 0.375rem; border-color: rgb(209, 213, 219); }
        .dark .choices__inner { background-color: rgb(55, 65, 81); border-color: rgb(75, 85, 99); color: #fff; }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.addEventListener('beforeunload', function(e) {
                if (!formSubmitted) {
                    ...
                    const formData = new FormData();
                    formData.append('_token', '{{ csrf_token() }}');
                    navigator.sendBeacon('{{ route("admin.upload.cleanup") }}', formData);
                }
            });
            ...
            if (form) {
                ...
                form.addEventListener('submit', function(e) {
                    if (formSubmitted) {
                        ...
                        fetch('{{ route("admin.upload.clear-tracking") }}', {
                            method: 'POST',
                            headers: {
            ...
            const cancelLink = document.querySelector('a[href="{{ route("admin.posts.index") }}"]');
            if (cancelLink) {
            ...
            if (defaultLangTitleInput && slugInput) {
                ...
                // Generate slug on page load if slug is empty but title is filled
                if (!slugInput.value && defaultLangTitleInput.value) {
                    generateSlug(defaultLangTitleInput.value, slugInput);
                }
            }
