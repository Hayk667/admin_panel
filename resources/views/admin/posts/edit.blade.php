    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" id="postForm" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            @if(isset($tags) && $tags->count() > 0)
                                <select name="tags[]" id="tags" multiple
                                    ...
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $post->tags->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $tag->getName() }}</option>
                                    @endforeach
                                </select>
                            @else
